...
def greedy_decode(model, tokenizer, input_text, max_length=1000):
...
def main():
    ...
    hedgehog_story = greedy_decode(model, tokenizer, hedgehog_prompt)
    print(hedgehog_story)
    print("\n" + "="*50 + "\n")

    print("Generating JSON:")
    json_output = greedy_decode(model, tokenizer, json_prompt)
    print(json_output)
